...
const getLocalCache = <T,>(key: string) => {
  try {
    ...
    if (age > (parsed.ttl ?? DEFAULT_CACHE_TTL_MS)) {
      if (isDev) console.debug(`[api/cache] expired: ${key} age=${age}ms ttl=${parsed.ttl}`);
...
const setLocalCache = <T,>(key: string, value: T, ttl = DEFAULT_CACHE_TTL_MS) => {
  try {
    const str = JSON.stringify({ ts: Date.now(), ttl, value });
    localStorage.setItem(`${CACHE_PREFIX}:${key}`, str);
    if (isDev) console.debug(`[api/cache] set: ${key} size=${str.length} ttl=${ttl}`);
  } catch (ex) {
...
const fetchWithSignal = async (url: string, opts: RequestInit = {}) => {
  ...
  if (!res.ok) {
    if (isDev) console.warn(`[api/fetch] ${res.status} ${res.statusText} ${url} (${time}ms)`);
...
const promisePool = async <T, R>(
  ...
  if (isDev) console.debug(`[api/pool] start workers=${concurrency} items=${items.length}`);
  const out: R[] = [];
  let i = 0;
  const run = async () => {
...
const fetchLatestHistory = async (stationId: string, signal?: AbortSignal) => {
  ...
  try {
    ...
    if (!res.ok) {
      if (isDev) console.warn(`[api/history] ${stationId} ${res.status} (${elapsed}ms)`);
      if (res.status === 429) throw new ApiError('Rate limited', 429);
      return { latestModified: null };
    }
    const data = await safeJson<{ presets?: { history?: HistoryEntry[] }[] }>(res);
    const history = data?.presets?.[0]?.history || [];
    if (history.length === 0) {
      if (isDev) console.debug(`[api/history] ${stationId} no history (${elapsed}ms)`);
      return { latestModified: null };
    }
    ...
    if (isDev) console.debug(`[api/history] ${stationId} latest=${latest.lastModified} (${elapsed}ms)`);
...
export const fetchStations = async (
  ...
  if (isDev) console.info(`[api/stations] start (cities=${cityOrCities ?? 'all'}) tick=${refreshTick}`);
  ...
  if (cached) {
    if (isDev) console.info(`[api/stations] cache hit key=${cacheKey} items=${cached.length}`);
    return cached;
  }
  ...
  try {
    ...
    const detailed = await promisePool(
      ...
      async (cam) => {
        ...
        try {
          if (isDev) console.debug(`[api/stations] fetching details for ${cam.id} "${cam.properties.name}"`);
          const detailsRes = await fetchWithSignal(`${API_BASE}/stations/${cam.id}`, {
            signal,
          });
          ...
          const presetId = details?.presets?.[0]?.id ?? cam.properties?.presets?.[0]?.id;
          const imageUrl =
          ...
          if (isDev) console.debug(
            `[api/stations] ${cam.id} details=${details ? 'ok' : 'null'} imageUrl=${!!imageUrl} latestModified=${latestModified}`
          );
          ...
          if (err instanceof ApiError) {
            if (isDev) console.error(`[api/stations] ${cam.id} ApiError`, err.status, err.message);
            throw err;
          }
          if (isDev) console.warn(`[api/stations] ${cam.id} fetch error`, err);
          return { cam, details: null, latestModified: null, imageUrl: undefined };
        }
      },
      concurrency
    );
    ...
    if (err instanceof ApiError) {
      if (isDev) console.error('[api/stations] ApiError', err.status, err.message);
      throw err;
    }
